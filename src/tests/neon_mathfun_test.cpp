...
union float_int_union {
  int i;
  float f;
} QNAN = { -0x400000 }, QNAN2 = { 0x7FC00000 }, PINF = { 0x7F800000 }, MINF = { -0x800000 };
...
//     v4sf x = set_ps1(0.5);                                              \
//     v4sf y = set_ps1(0.0);                                              \
//     const v4sf epsilon = set_ps1(1e-6f);                                \
//     for (i=0; i < niter; ++i) {                                         \
//       x += epsilon; y += fn(x);                                         \
//     }                                                                   \
//     V4SF xx; xx.v = y;                                                  \
...
float32x4_t Vtesting_taylor(float32x4_t x){
  const float32x4_t center = vmovq_n_f32(M_PI/6), precomputed_sin = vdupq_n_f32(0.5),precomputed_cos = vdupq_n_f32(0.866025403784);
  v4sf s, c;
  Vsincos_taylor(x,center, precomputed_sin, precomputed_cos, &s, &c);
  return s + c;
}

float32x4_t Vtesting_taylor_delta(float32x4_t x){
  const float32x4_t precomputed_sin = vdupq_n_f32(0.5),precomputed_cos = vdupq_n_f32(0.866025403784);
  v4sf s, c;
  Vsincos_taylor_delta(x, precomputed_sin, precomputed_cos, &s, &c);
  return s + c;
}

float testing_taylor(float x){
  const float center = M_PI/6, precomputed_sin = 0.5,precomputed_cos = 0.866025403784;
  float s, c;
  sincos_taylor(x,center, precomputed_sin, precomputed_cos, &s, &c);
  return s + c;
}

float testing_taylor_delta(float x){
  const float precomputed_sin = 0.5, precomputed_cos = 0.866025403784;
  float s, c;
  sincos_taylor_delta(x, precomputed_sin, precomputed_cos, &s, &c);
  return s + c;
}
...
DECL_SCALAR_FN_BENCH(testing_taylor);
DECL_SCALAR_FN_BENCH(testing_taylor_delta);
DECL_VECTOR_FN_BENCH(sin_ps);
DECL_VECTOR_FN_BENCH(cos_ps);
DECL_VECTOR_FN_BENCH(stupid_sincos_ps);
DECL_VECTOR_FN_BENCH(Vtesting_taylor);
DECL_VECTOR_FN_BENCH(Vtesting_taylor_delta);
...
void run_bench(const char *s, int (*fn)()) {
  ...
  // niter - total number of evaluations performed
  double niter = 0;
  ...
  it1 = pros::micros();
  double t0 = ((double)it0)/1000000.0,t1 = ((double)it1)/1000000.0;
  #define REF_FREQ_MHZ 667.0
  printf(" -> %6.2f millions of vector(4 values) evaluations/second -> %3.2f cycles/value on a %gMHz computer\n", (niter/(t1-t0)) / 1e6, ((t1-t0)*(REF_FREQ_MHZ*1e6)/niter)/4, REF_FREQ_MHZ);
}
...
int neon_mathfun_test(){
  ...
  run_bench("testing_taylor", bench_testing_taylor);
  run_bench("testing_taylor_delta", bench_testing_taylor_delta);

  run_bench("sin_ps", bench_sin_ps);
  run_bench("cos_ps", bench_cos_ps);
  run_bench("sincos_ps", bench_stupid_sincos_ps);
  run_bench("Vtesting_taylor", bench_Vtesting_taylor);
  run_bench("Vtesting_taylor_delta", bench_Vtesting_taylor_delta);
